...
Color StripePattern::patternAt(const Tuple& patternPoint) const {
    if (static_cast<int>(std::floor(patternPoint.getX())) % 2 == 0) {
        return m_firstColor;
    }
...
Color GradientPattern::patternAt(const Tuple& patternPoint) const {
    ...
    const double frac = patternPoint.getX() - std::floor(patternPoint.getX());
...
Color RingPattern::patternAt(const Tuple& patternPoint) const {
    ...
    if (static_cast<int>(std::floor(distance)) % 2 == 0) {
        return m_firstColor;
    }
    return m_secondColor;
}
...
Color Checker3DPattern::patternAt(const Tuple& patternPoint) const {
    const int sum = static_cast<int>(std::floor(patternPoint.getX())) +
                    static_cast<int>(std::floor(patternPoint.getY())) +
                    static_cast<int>(std::floor(patternPoint.getZ()));
    if (sum % 2 == 0) {
        return m_firstColor;
    }
    return m_secondColor;
